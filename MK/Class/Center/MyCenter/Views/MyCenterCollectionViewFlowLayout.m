...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        attributes.frame = CGRectMake(K_Padding_Home_LeftPadding+(self.section1ItemSize.width+MyCenterCollectionViewHorizontalPadding)*(indexPath.row%2), KScaleHeight(25)+(self.section1ItemSize.height+MyCenterCollectionViewVertialPadding)*(indexPath.row/2), self.section1ItemSize.width, self.section1ItemSize.height);
    }else{
        double numSection1Item = (double)[self.collectionView numberOfItemsInSection:0];
        attributes.frame = CGRectMake(K_Padding_Home_LeftPadding+(self.section2ItemSize.width+MyCenterCollectionViewHorizontalPadding)*(indexPath.row%3), KScaleHeight(20) +(ceil(numSection1Item/2))*(self.section1ItemSize.height+MyCenterCollectionViewVertialPadding)+KScaleHeight(24)+(self.section2ItemSize.height+MyCenterCollectionViewVertialPadding)*(indexPath.row/3), self.section2ItemSize.width, self.section2ItemSize.height);
    }
    return  attributes;
}
