...
@protocol PLVSimplePingDelegate;

/*! Controls the IP address version used by PLVSimplePing instances.
 */

typedef NS_ENUM(NSInteger, PLVSimplePingAddressStyle) {
    PLVSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    PLVSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    PLVSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface PLVSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<PLVSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) PLVSimplePingAddressStyle addressStyle;
...
/*! A delegate protocol for the PLVSimplePing class.
 */

@protocol PLVSimplePingDelegate <NSObject>

@optional

/*! A PLVSimplePing delegate callback, called once the object has started up.
...
/*! A PLVSimplePing delegate callback, called if the object fails to start up.
 *  \details This is called shortly after you start the object to tell you that the 
...
/*! A PLVSimplePing delegate callback, called when the object has successfully sent a ping packet. 
...
/*! A PLVSimplePing delegate callback, called when the object fails to send a ping packet. 
 *  \details Each call to `-sendPingWithData:` will result in either a 
 *      `-simplePing:didSendPacket:sequenceNumber:` delegate callback or a 
 *      `-simplePing:didFailToSendPacket:sequenceNumber:error:` delegate callback (unless you 
 *      stop the object before you get the callback).  These callbacks are currently delivered 
...
/*! A PLVSimplePing delegate callback, called when the object receives a ping response.
...
/*! A PLVSimplePing delegate callback, called when the object receives an unmatched ICMP message.
