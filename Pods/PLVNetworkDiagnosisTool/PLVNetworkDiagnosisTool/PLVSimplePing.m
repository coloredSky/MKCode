...
- (void)didFailWithError:(NSError *)error {
    id<PLVSimplePingDelegate>  strongDelegate;
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge PLVSimplePing *) info;
    assert([obj isKindOfClass:[PLVSimplePing class]]);
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        if (self.addressStyle != PLVSimplePingAddressStyleICMPv6) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
                    case AF_INET6: {
                        if (self.addressStyle != PLVSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
